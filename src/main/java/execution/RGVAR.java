...
public class RGVAR {
    ...
    BoolExpr prove(Context ctx, PP P, QQ Q, relyRelation R, GG G, consistency I, CC c, Expr effect_set) throws Z3Exception {
...
//        Expr b1=ctx.mkSetSubset(a.effect_set, effect_set);
        ...
        Expr b3=(BoolExpr) P.get(ctx, effect_set);
        ...
        Expr[] arg =  new Expr[3];
        arg[0]=effect;
        arg[1]= application.type.getConst(1);
        arg[2]= obj;
        ...
        BoolExpr b=ctx.mkAnd(new BoolExpr[] {(BoolExpr) b1, (BoolExpr) b2,(BoolExpr) b3, (BoolExpr) b4, (BoolExpr) ee1,(BoolExpr) ee2,(BoolExpr) ee3, (BoolExpr) ee4, (BoolExpr) ee5});

        Expr r1=Q.get(ctx, effect_set2);
        Expr r2=G.get(ctx, effect_set, effect_set2);
        Expr r3=c.get(ctx,rval);
        BoolExpr r=ctx.mkAnd(new BoolExpr[] {(BoolExpr) r1,(BoolExpr) r2,(BoolExpr) r3});
        ...
        s.add(ctx.mkNot(stable));
        Status status=s.check();

        System.out.println("assertion: \n"+s.getAssertions()[0]);
     //   try {
            //Model m = t.Check(ctx,ctx.mkNot(stable), Status.SATISFIABLE);
           // System.out.println("model:"+m);
        //} catch (SequentialCheck.TestFailedException e) {
